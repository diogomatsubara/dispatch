...
DISPATCH_HEADER_AUTH_ENABLED = config("DISPATCH_HEADER_AUTH_ENABLED", cast=bool, default=False)
...
if DISPATCH_HEADER_AUTH_ENABLED:
    if not DISPATCH_AUTH_HEADER_KEY:
        raise Exception("DISPATCH_AUTH_HEADER must be set when DISPATCH_HEADER_AUTH_ENABLED")
